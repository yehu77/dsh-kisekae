...
- 通过“固定／随机／关闭”让已经发布的主 `conversation.backdrop` 始终保持可选。Hero 使用 82% 的全幅图片、较轻的中央航道和输入区附近的语义海雾，不叠加顶部遮罩。Active 在 88% 的全幅图片上叠加居中且最大 900px 宽的航道；其 92% 的语义基础色强保护核心在两侧渐隐，120px 顶部遮罩保护会话标题栏控件，却不会洗白整张画布。Settling 使用 64% 的图片、88% 的航道核心和同类顶部遮罩，且不使用模糊。
