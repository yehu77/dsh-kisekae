...
- Mount the conversation backdrop, sidebar backdrop, New Session decoration, wave-chat glyph, and Settings trigger decoration only while Blue Whale-chan is the visible draft. Official preview removes the group, and Cancel restores the saved skin's group without clearing its preferences.
...
- Extend the theme later to composer styling as a separately optional surface. Conversation and sidebar backgrounds already use public backdrop slots; reuse the artwork catalog through role-specific browser copies instead of stretching every illustration into every surface.
...
- The product-neutral `conversation.backdrop`, `sidebar.backdrop`, `sidebar.newSession.decoration`, `sidebar.newSession.icon`, and `settings.trigger.decoration` slots carry main-canvas artwork, sidebar artwork, and themed button treatments. Any other visual surface still needs its own generic seam rather than private selectors.
