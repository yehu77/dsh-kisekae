...
Main-background controls save immediately under `@yehu77/dsh-kisekae:main-background:v1`. Random chooses one illustration per page load, Fixed uses the image selected in the gallery, and Off removes the background. Hero uses 82% full-bleed artwork with a lighter central lane, semantic sea fog near the composer, and no top veil. Active keeps the full-bleed image at 88%; a centered lane capped at 900px puts a 92% semantic base-color core behind the transcript, fades near both edges, and adds a 120px veil for the conversation header controls. These protections stay local instead of washing the whole canvas pale. Settling uses 64% artwork with an 88% lane core and matching top veil, without blur. The background appears only while Blue Whale-chan is visible, while its preference remains available during Official appearance preview. The 104 MiB source library remains untouched; the plugin ships about 22 MiB of source-resolution JPEG copies and lazy-loads gallery images.
