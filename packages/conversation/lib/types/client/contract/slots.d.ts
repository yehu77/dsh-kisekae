...
declare module '@deepseek-ai/dsh-client-ui-slots' {
    interface SlotMap {
        /**
         * The non-interactive visual layer behind the resident conversation shell
         * across settling, hero, and active phases. ConversationRoot owns clipping,
         * stacking, pointer suppression, and accessibility isolation; the occupant
         * receives only the current presentation phase and owns its artwork.
         * Root scope keeps one occupant mounted across current-session switches.
         */
        'conversation.backdrop': {
            kind: 'single';
            scope: 'root';
            owner: ConversationBackdropOwnerProps;
        };
        /**
        ...
        /**
         * The visual layer behind the official composer card's textarea and
         * controls. InputBar clips and isolates the layer while retaining the
         * card geometry, interaction, accessibility, and empty-seat presentation.
         * Root scope keeps one occupant mounted across current-session switches.
         */
        'conversation.composer.bar.decoration': {
            kind: 'single';
            scope: 'root';
            owner: ComposerBarDecorationOwnerProps;
        };
        /**
...
/** Presentation phases dispatched to the resident conversation backdrop. */
export type ConversationBackdropPhase = 'settling' | 'hero' | 'active';
/** Owner share of the conversation shell's non-interactive backdrop seat. */
export interface ConversationBackdropOwnerProps {
    /** Current resident-shell presentation phase. */
    phase: ConversationBackdropPhase;
}
...
/** Owner share of the official composer card's decorative layer. */
export interface ComposerBarDecorationOwnerProps {
...
export type ComposerBarProps = PropsRuntime<'conversation.composer.bar'> & PropsRenderSlots<'conversation.composer.bar.decoration' | 'conversation.input.plan' | 'conversation.input.model'> & InjectFace<ComposerBarInjected> & PropsLocale<'conversation'>;
/**
...
export type ConversationSessionHeaderSlotProps = PropsRuntime<'conversation.session.header'> & PropsRenderSlots<'conversation.session.header.actions' | 'conversation.session.header.utilities'> & PropsStore<ChatStore> & ConversationSessionHeaderInjected & PropsLocale<'conversation'>;
