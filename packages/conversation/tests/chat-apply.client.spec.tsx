...
describe('apply wiring', () => {
  ...
  it('occupies the slots + the ring; session entries share one store handle', async () => {
    ...
    expect(b.slots.spec('conversation.backdrop')).toEqual({ kind: 'single', scope: 'root' })
    expect(b.slots.spec('conversation.composer.bar.decoration')).toEqual({ kind: 'single', scope: 'root' })
  ...
  it('plugin fiber disposal collects every registration (unload cascade, ring and hole included)', async () => {
    const b = await bench()
    b.slots.register({ name: 'conversation.backdrop' }, () => null)
    b.slots.register({ name: 'conversation.composer.bar.decoration' }, () => null)
    expect(b.slots.entries('conversation.backdrop')).toHaveLength(1)
    expect(b.slots.entries('conversation.composer.bar.decoration')).toHaveLength(1)
    ...
    expect(b.slots.entries('conversation.backdrop')).toHaveLength(0)
    expect(b.slots.spec('conversation.backdrop')).toBeUndefined()
    expect(b.slots.entries('conversation.composer.bar.decoration')).toHaveLength(0)
    expect(b.slots.spec('conversation.composer.bar.decoration')).toBeUndefined()
