...
import type { ComposerAttachment, ComposerBarDecorationOwnerProps } from '../src/client/contract/slots.ts'
...
interface BenchOptions {
  decorationEntry?: React.ReactNode
...
function bench(over?: BenchOptions) {
  ...
  const renderSlot = ((key: string, owner: object) => {
    ...
    if (key === 'conversation.composer.bar.decoration') return over?.decorationEntry ?? null
...
describe('strips and variants', () => {
  ...
  it('layers a phase-aware decoration behind the official card content and send behavior', () => {
    const { view, textarea, button, sink, slotCalls } = bench({
      draft: 'hello',
      variant: 'hero',
      decorationEntry: <i data-testid="card-decoration" />,
    })
    const decoration = view.getByTestId('card-decoration')
    const decorationArea = decoration.parentElement
    const card = decoration.closest('[data-composer-card]')
    expect(decorationArea?.getAttribute('aria-hidden')).toBe('true')
    expect(decorationArea?.nextElementSibling?.contains(textarea)).toBe(true)
    expect(card?.contains(button)).toBe(true)
    expect(textarea.closest('[aria-hidden="true"]')).toBeNull()
    expect(slotCalls.find(call => call.key === 'conversation.composer.bar.decoration')?.owner)
      .toEqual({ variant: 'hero' } satisfies ComposerBarDecorationOwnerProps)
    fireEvent.click(button)
    expect(sink).toHaveBeenCalledWith('hello', [], 'queue')
  })
...
describe('command launcher chrome and control seats', () => {
  it('renders the command launcher; the Access chip is absent without the permissions projection; the control seats render EMPTY without entries', () => {
    ...
    expect(slotCalls.map(c => c.key)).toEqual([
      'conversation.composer.bar.decoration', 'conversation.input.plan', 'conversation.input.model',
    ])
  ...
  it('a registered entry fills its seat and receives the locked owner prop', () => {
    ...
    expect(slotCalls.filter(c => c.key !== 'conversation.composer.bar.decoration')
      .every(c => (c.owner as { locked: boolean }).locked)).toBe(true)
    cleanup()
    const live = bench({ running: true })
    expect(live.slotCalls.filter(c => c.key !== 'conversation.composer.bar.decoration')
      .every(c => !(c.owner as { locked: boolean }).locked)).toBe(true)
  })
