...
import type {
  ComposerBarOwnerProps, ConversationBackdropOwnerProps,
...
function mount(
  ...
  let backdropOwner: ConversationBackdropOwnerProps | undefined
  let pickerOwner: unknown
  const renderSlot = ((key: string, owner: object, opts?: { only?: string }) => {
    slotCalls.push(key)
    if (key === 'conversation.backdrop') {
      backdropOwner = owner as ConversationBackdropOwnerProps
      return <div data-testid="conversation-backdrop" />
    }
  ...
  return {
    ...
    backdropOwner: () => {
      if (backdropOwner === undefined) throw new Error('backdrop owner not rendered')
      return backdropOwner
    },
...
describe('ConversationRoot resident composer', () => {
  it('isolates the phase-bound backdrop below the resident content', () => {
    const b = mount(conversationSnapshot())
    expect(b.backdropOwner()).toEqual({ phase: 'active' })
    const backdropArea = b.view.getByTestId('conversation-backdrop').parentElement
    expect(backdropArea?.getAttribute('aria-hidden')).toBe('true')
    expect(backdropArea?.nextElementSibling?.contains(b.view.getByRole('textbox'))).toBe(true)
  })
  ...
  it('settling phase: a summary that does not prove the session blank hides the composer while it opens', () => {
    ...
    expect(b.backdropOwner().phase).toBe('settling')
  ...
  it('startup auto-selection: a summary-proven blank session opens straight into the hero', () => {
    ...
    expect(b.backdropOwner().phase).toBe('hero')
    expect(b.view.getByText('探索未至之境')).toBeTruthy()
  ...
  it('same textarea DOM node survives the hero → active flip into the sticky scrollport', () => {
    ...
    expect(b.backdropOwner().phase).toBe('active')
  })
