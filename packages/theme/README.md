...
The first working skin is “DeepSeek Blue Whale-chan,” an unofficial community theme built around cool ocean daylight and a deep-sea night palette. Installing the package immediately overlays 23 semantic color tokens, four composer-surface tokens, two role-scoped conversation text-shadow tokens, and one shared conversation font-weight token while preserving the official Harness Light, Dark, and System preferences. The settings page also includes a 42-image gallery, a phase-aware main conversation background, the “Blue Whale glass sidebar · Rain veil,” and themed composer, New Session, and Settings surfaces.
...
The same page includes an immediate **Text style** preference. **Blue Whale Ice** follows the active theme's default typography, **Official clear** restores neutral official prose and regular weight, and **Turn off text effects** keeps the theme's message colors and weight without outlines or shadows. The choice is saved for this browser under `@yehu77/dsh-kisekae:text-style:v1`. Future themes can supply their own default font family and effects while these two generic overrides remain available.

Main-background controls save immediately under `@yehu77/dsh-kisekae:main-background:v1`. Random chooses one illustration per page load, Fixed uses the image selected in the gallery, and Off removes the background. The native 3840×2160 night-ocean wallpaper uses full-opacity `cover` to fill the conversation canvas without a duplicate ambient layer. Other illustrations keep the complete uncropped composition at full opacity with `contain`, over an unblurred low-opacity `cover` copy that fills unused canvas. Text readability belongs to separate content surfaces rather than a backdrop-wide wash. The background appears only while Blue Whale-chan is visible, while its preference remains available during Official appearance preview. The 112 MiB source library remains untouched; the plugin ships about 25 MiB of source-resolution JPEG copies and lazy-loads gallery images.
...
The composer keeps the official textarea, controls, focus, file-drop, and resizing behavior. Blue Whale-chan paints only the card's non-interactive background: mode-aware ocean glass, a double luminous rim and top highlight, three tide lines, paired whale-tail corners, control halos, pearls, a star, and a shell. Hero uses the strongest treatment; the resident composer keeps the same identity at slightly lower ornament intensity. Behind the resident card, a translucent deep-sea dock mask replaces the opaque base-color mask so the wallpaper continues to the viewport edge. Every ornament is scalable CSS or inline SVG, with no raster artwork or blur.
...
The project is developed against DeepSeek Harness commit `18c22363d7c8655603a065909c05c5222736d5d1` (`0.1.0-rc.5`), which exposes `conversation.composer.bar.decoration`, `conversation.backdrop`, `settings.trigger.decoration`, `sidebar.backdrop`, `sidebar.newSession.decoration`, and `sidebar.newSession.icon`, plus separate user and assistant message-prose color and text-shadow tokens and their shared font-weight token. The sibling [`packages/conversation`](../conversation) package distributes the complete replacement conversation base with those capabilities. DeepSeek Harness is still in developer preview, so both packages pin and review compatibility together rather than assuming it.

See the [workspace roadmap](../../ROADMAP.md) for product scope, delivery stages, decision gates, and release criteria.
...
This package owns the complete skin definition, including typography. Each future theme may provide its own message colors, text effects, weights, and licensed font families. The sibling conversation package remains theme-neutral and only exposes the places where those values are consumed.
...
Install the conversation base first; it keeps the official package identity so the Web profile resolves it in place of Harness's bundled conversation plugin. Both packages come from this repository through pnpm's Git `path:` selector and commit their built artifacts, so installation needs no build permission. The CLI's notice that the conversation package has no `dsh.bundle` is expected because the existing `ui-conversation` configuration row already mounts that package identity. Pin a common tag or commit for a reproducible installation.

## Repository layout

```text
src/index.ts          Host entry and same-origin artwork route
src/artworks.ts       Shared 42-image catalog and per-image canvas fit
...
src/client/text-style-store.ts  Immediate theme-default, official-clear, and effects-off preference
...
../../assets/inbox/  User-managed read-only source library outside the package
