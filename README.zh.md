...
主背景选项会立即保存到 `@yehu77/dsh-kisekae:main-background:v1`。“随机”在每次加载页面时选择一张图片，“固定”使用图鉴中点选的图片，“关闭”则移除背景。Hero 阶段使用 82% 的全幅图片，配合较轻的中央航道和输入区附近的语义海雾，不叠加顶部遮罩。Active 阶段保留 88% 的高强度全幅图片；居中且最大 900px 宽的阅读航道在 transcript 后方放置 92% 的语义基础色强保护核心，并在两侧渐隐，同时用 120px 顶部遮罩保护会话标题栏控件。这些保护层只位于需要的区域，不会把整张画布洗成浅色。Settling 阶段使用 64% 的图片、88% 的航道核心和同类顶部遮罩，且不使用模糊。主背景只在蓝鲸娘可见时出现；预览官方外观期间，其偏好仍会保留。104 MiB 原始素材保持不变；插件使用约 22 MiB 的原始像素尺寸 JPEG 发布副本，并在图鉴中懒加载。
